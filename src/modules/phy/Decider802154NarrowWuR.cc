/*
 * Decider802154NarrowWuR.cc
 *
 *  Created on: 28 août 2020
 *      Author: asus
 */
#include "Decider802154NarrowWuR.h"
...
bool Decider802154NarrowWuR::initFromMap(const ParameterMap& params) {
...
bool Decider802154NarrowWuR::syncOnSFD(airframe_ptr_t frame) const {
    double BER;
...
double Decider802154NarrowWuR::evalBER(airframe_ptr_t frame) const {
...
/*simtime_t Decider802154NarrowWuR::processSignalHeader(airframe_ptr_t frame)
{
    if (!syncOnSFD(frame)) {
...
DeciderResult* Decider802154NarrowWuR::createResult(const airframe_ptr_t frame) const
{
...
double Decider802154NarrowWuR::n_choose_k(int n, int k) {
    if (n < k)
        return 0.0;
...
double Decider802154NarrowWuR::getBERFromSNR(double snr) const {
